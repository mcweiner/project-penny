...
for n in range(10):
    yield_file_name = f'shuffled_lists_yield_part_{n+1}.npy'
    iter_file_name = f'shuffled_lists_iter_part_{n+1}.npy'
    folder_name = "Data"
    yield_file_path = os.path.join(folder_name, yield_file_name)
    iter_file_path = os.path.join(folder_name, iter_file_name)
    
    yield_file_size.append(os.path.getsize(yield_file_path))
    iter_file_size.append(os.path.getsize(iter_file_path))
    #print(iter_file_size)
...
def dataframe_to_markdown_file(df: pd.DataFrame, filename: str):
    """
    Converts a pandas DataFrame into a markdown-formatted table and saves it to a file.
    ...
    try:
        ...
        # The 'tablefmt' parameter ensures a standard markdown table.
        # The 'index=False' prevents the DataFrame index from being written.
        markdown_table = df.to_markdown(index=False)
        
        # Write the markdown string to the specified file.
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(markdown_table)
        print(f"Successfully created markdown file: {filename}")
    except Exception as e:
        print(f"An error occurred: {e}")


output_filename = 'DataGeneration.md'
# Call the function to create the markdown file.
dataframe_to_markdown_file(df, output_filename)
